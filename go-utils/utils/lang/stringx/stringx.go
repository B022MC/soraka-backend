...
func GeneratePinyin(name string) (string, string) {
	args := pinyin.NewArgs()
	fullPinyin := pinyin.LazyPinyin(name, args)
	firstLetters := make([]string, len(fullPinyin))
	for i, s := range fullPinyin {
		firstLetters[i] = string(s[0])
	}

	return strings.Join(fullPinyin, ""), strings.Join(firstLetters, "")
}
